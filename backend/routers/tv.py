"""
Router de TV — endpoints ETL para datos de televisión
...
from sqlalchemy import (
    func,
)  # func: Funciones SQL para consultas (count, min, max, sum, etc.)
...
from services.auditsa_tv_etl_service import AuditsaTvEtlService
...
@router.get(
    "/stats",
    summary="Estadísticas generales de TV en base de datos",
    description="Obtiene estadísticas generales de TV en base de datos.",
)
async def get_stats(db: Session = Depends(get_db), _: dict = Depends(require_media_access("tv"))):
    cached = stats_cache.get("tv_stats")
    if cached:
        return cached
    result = db.query(
        func.count(db_auditsa_tv.id).label("total_records"),
        func.count(func.distinct(db_auditsa_tv.Fecha)).label("unique_dates"),
        func.count(func.distinct(db_auditsa_tv.IdCanal)).label("unique_channels"),
        func.count(func.distinct(db_auditsa_tv.IdMarca)).label("unique_brands"),
        func.count(func.distinct(db_auditsa_tv.IdAnunciante)).label(
            "unique_advertisers"
        ),
        func.min(db_auditsa_tv.Fecha).label("first_date"),
        func.max(db_auditsa_tv.Fecha).label("last_date"),
        func.coalesce(func.sum(db_auditsa_tv.Tarifa), 0).label("total_tarifa"),
    ).first()  # first(): Retorna la primera fila de la consulta o None si está vacía.
    data = {
        "tipo": "tv",
    ...
    stats_cache.set(  # set(): Almacena en caché los datos con clave "tv_stats".
        "tv_stats", data
    )
    return data
...
@router.get(
    "/top-marcas",
    summary="Top marcas de TV por inversión (todos los tiempos)",
    description="Obtiene las top marcas de TV por inversión (todos los tiempos).",
)
async def get_top_marcas(
    limit: int = Query(default=10, ge=1, le=20),
    db: Session = Depends(get_db),
    _: dict = Depends(require_media_access("tv")),
):
    cached = stats_cache.get("tv_top_marcas")
    if cached:
        return cached
    rows = (
        db.query(
            ...
            db_auditsa_tv.Marca,
            func.count(db_auditsa_tv.id).label("registros"),
            func.coalesce(func.sum(db_auditsa_tv.Tarifa), 0).label("total_inversion"),
        )
        .filter(
            db_auditsa_tv.Marca.notin_(["N.A.", ""]),
        )
        .group_by(
            db_auditsa_tv.Anunciante,
            db_auditsa_tv.Marca,
...
@router.get(
    "/top-sectores",
    summary="Top sectores de TV por inversión (todos los tiempos)",
)
async def get_top_sectores(
    ...
    cached = stats_cache.get("tv_top_sectores")
    if cached:
        return cached
    rows = (
        db.query(
        ...
        .filter(db_auditsa_tv.Industria.notin_(["N.A.", ""]))
...
@router.get("/{fecha}", summary="Obtener datos de TV desde Auditsa API")
async def get_data(
    fecha: str,
    limit: Optional[int] = Query(
        default=None, description="Límite de registros a devolver"
    ),
    _: dict = Depends(require_media_access("tv")),
):
    try:
        api = AuditsaApiService()
        data = api.get_tv_data(fecha)
        if limit:
            data = data[:limit]
        return {"fecha": fecha, "total": len(data), "data": data}
    except Exception as e:
        logger.error(f"Error obteniendo datos TV para {fecha}: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get(
    "/range/{fecha_inicio}/{fecha_fin}",
    summary="Obtener datos de TV para un rango de fechas",
)
async def get_data_range(
    fecha_inicio: str, fecha_fin: str, _: dict = Depends(require_media_access("tv"))
):
    try:
        etl = AuditsaTvEtlService()
        ...
        logger.error(f"Error obteniendo datos TV rango {fecha_inicio}-{fecha_fin}: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/etl", summary="Ejecutar ETL de TV para una fecha")
# Limitar a 10 solicitudes por minuto para evitar sobrecarga en la API de Auditsa y en el sistema de ingestión.
# El ETL es un proceso pesado, así que es importante controlar la frecuencia de ejecución.
...
def run_etl(
    ...
    3. Upload a S3 — la S3 Event Notification dispara automáticamente al topic SNS
       → SNS hace fan-out a tv-spark-queue → Spark Consumer procesa asincrónamente
    ...
    El Spark Consumer usa delete-before-insert, por lo que re-ejecutar con force es seguro.
    """
    etl = AuditsaTvEtlService()
    s3 = S3Service()
    ...
    try:
        ...
        if not force:
            existing_count = db.query(func.count(db_auditsa_tv.id)).filter(
                db_auditsa_tv.Fecha == datetime.strptime(data.fecha, "%Y%m%d").date()
            ).scalar()
            if existing_count > 0:
                raise HTTPException(
                    status_code=409,
                    detail=f"Ya existen {existing_count} registros de TV para fecha={data.fecha}. "
                           f"Usa force=true para re-ingestar (Spark eliminará los datos previos automáticamente).",
                )

        raw_data = etl.extract_tv_data(data.fecha)
        total = len(raw_data)

        s3_key = f"rawdata/tv/{data.fecha}/data.json"
        ...
        return {
            ...
            "s3_uri": s3_uri,
            "idempotent": True,
            "message": "Evento DataIngested registrado. S3 notificó a SNS → tv-spark-queue. Spark Consumer procesará asincrónamente.",
        }
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"ETL TV error para {data.fecha}: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/etl/range", summary="Ejecutar ETL de TV para un rango de fechas")
...
async def run_etl_range(
    ...
    def process_fecha(fecha: str) -> dict:
        ...
        try:
            # Idempotencia: verificar si ya existen datos para esta fecha
            if not force:
                existing_count = thread_db.query(func.count(db_auditsa_tv.id)).filter(
                    db_auditsa_tv.Fecha == datetime.strptime(fecha, "%Y%m%d").date()
                ).scalar()
                if existing_count > 0:
                    return {
                        "fecha": fecha,
            ...
            raw_data = etl.extract_tv_data(fecha)
            total = len(raw_data)
            s3_key = f"rawdata/tv/{fecha}/data.json"
            s3_uri = f"s3://{s3.bucket}/{s3_key}"
            ...
            logger.error(f"ETL TV range error para {fecha}: {e}")
    ...
    total_extracted = sum(
        r.get("records", 0) for r in results if r["status"] != "error"
    )
    return {
        "status": "accepted",
        ...
        "dates": results,
        "message": f"Ingesta paralela ({ETL_WORKERS} workers). SNS notificó a tv-spark-queue. Spark Consumer procesará asincrónamente.",
    }


@router.get("/status/range", summary="Estado de datos de TV para un rango con filtros")
async def get_status_range(
    ...
    # TV no tiene Categoria → si ese filtro está activo, sin datos
    ...
    cache_key = stats_cache.make_key(
        ...
        industria, sector_industria, submarca,
    )
    ...
    q = db.query(
        ...
        db_auditsa_tv.Fecha >= fi,
        db_auditsa_tv.Fecha <= ff,
    )
    ...
    if anunciante_list:
        q = q.filter(db_auditsa_tv.Anunciante.in_(anunciante_list))
    if marca_list:
        q = q.filter(db_auditsa_tv.Marca.in_(marca_list))
    if medio_list:
        q = q.filter(db_auditsa_tv.Medio.in_(medio_list))
    if tipo_medio_list:
        q = q.filter(db_auditsa_tv.TipoMedio.in_(tipo_medio_list))
    if localidad_list:
        q = q.filter(db_auditsa_tv.Localidad.in_(localidad_list))
    submarca_list = parse_csv(submarca)
    if submarca_list:
        q = q.filter(db_auditsa_tv.Submarca.in_(submarca_list))
    industria_list = parse_csv(industria)
    if industria_list:
        q = q.filter(db_auditsa_tv.Industria.in_(industria_list))
    # sector_industria es el campo unificado TV+Radio+Impresos → mapea a Industria en TV
    sector_industria_list = parse_csv(sector_industria)
    if sector_industria_list:
        q = q.filter(db_auditsa_tv.Industria.in_(sector_industria_list))
...
@router.get(
    "/top-marcas/range", summary="Top marcas de TV por inversión en un rango de fechas"
)
async def get_top_marcas_range(
    ...
    rows = (
        q.order_by(func.coalesce(func.sum(db_auditsa_tv.Tarifa), 0).desc())
        .limit(limit)
        .all()
    )
    data = [
...
@router.get(
    "/top-sectores/range",
    summary="Top sectores de TV por inversión en un rango de fechas",
)
async def get_top_sectores_range(
    ...
    q = (
        ...
        .filter(
            db_auditsa_tv.Fecha >= fi,
            db_auditsa_tv.Fecha <= ff,
            db_auditsa_tv.Industria.notin_(["N.A.", ""]),
        )
        .group_by(db_auditsa_tv.Industria)
    )
    for val, col in [
        ...
        (sector_industria, db_auditsa_tv.Industria),
    ]:
        vals = parse_csv(val)
        if vals:
...
@router.get("/status/{fecha}", summary="Estado de datos de TV para una fecha")
async def get_status(fecha: str, _: dict = Depends(require_media_access("tv"))):
    etl = AuditsaTvEtlService()
    return etl.get_etl_status(fecha)


@router.delete("/range", summary="Eliminar datos de TV para un rango")
async def delete_data_range(
    ...
    try:
        ...
        log_db = log.start(
            source="public.auditsa_api_tv", instruction="DELETE", total_rows=0
        )
        db_result = etl.delete_tv_data_range(data.fecha_inicio, data.fecha_fin, db)
        ...
        event_id = es.publish(
            ...
            payload={
                ...
                "db_records_deleted": db_result["records_deleted"],
            },
        )
...
@router.post(
    "/fix-nulls",
    summary="Normalizar valores inválidos en todas las columnas de auditsa_api_tv",
)
async def fix_nulls(db: Session = Depends(get_db), _: dict = Depends(require_admin)):
    """
    Recorre auditsa_api_tv y reemplaza por 'N.A.' todos los registros donde
...
@router.delete("/{fecha}", summary="Eliminar datos de TV para una fecha")
async def delete_data(
    fecha: str, db: Session = Depends(get_db), _: dict = Depends(require_admin)
):
    etl = AuditsaTvEtlService()
    s3 = S3Service()
    sns = SNSService()
    ...
    try:
        ...
        db_result = etl.delete_tv_data(fecha, db)
        log.complete(log_db, total_records=db_result["records_deleted"])
        ...
        sns.publish(
            SNS_TOPIC,
            "DataDeleted",
            {"tipo": "tv", "fecha": fecha, "event_id": event_id},
        )
