"""
Servicio ETL especializado para datos de Radio de Auditsa
Maneja Extract, Transform, Load para información de radio
"""
...
class AuditsaRadioEtlService:
    """Servicio ETL para datos de Radio de Auditsa"""

    def __init__(self):
        self.logger = logging.getLogger("services.auditsa_radio_etl_service")
        self.api_service = AuditsaApiService()

        # IDs de medios para Radio (definidos en main.py)
        self.radio_ids = [1, 2]  # Radio IDs de Auditsa
    ...
    def _convert_fecha_to_date(self, fecha_str: str) -> Optional[date]:
        ...
        Returns:
            Objeto date de Python o None si no se puede convertir
        """
        ...
        try:
            ...
            if len(numbers) >= 3:
                ...
                if (
                    ...
                    return datetime(
                        year, month, day
                    ).date()  # Retornar objeto date con formato: YYYY-MM-DD
    ...
    def extract_radio_data(self, fecha: str) -> List[Dict]:
        """
        Extract: Extrae datos de Radio desde la API de Auditsa

        Args:
            fecha: Fecha en formato YYYYMMDD

        Returns:
            Lista de diccionarios con datos crudos de Radio
        """
        try:
            self.logger.info(
                f"📥 EXTRACT: Iniciando extracción de datos Radio para fecha {fecha}"
            )

            all_radio_data = []

            # Obtener datos para cada ID de medio de Radio
            for radio_id in self.radio_ids:
                self.logger.info(f"📥 EXTRACT: Consultando Radio ID {radio_id}")

                try:
                    radio_data = self.api_service.get_radio_data(fecha, radio_id)
                    if radio_data:
                        all_radio_data.extend(radio_data)
                        self.logger.info(
                            f"📥 EXTRACT: {len(radio_data)} registros de Radio ID {radio_id}"
                        )
                    else:
                        self.logger.warning(
                            f"📥 EXTRACT: Sin datos para Radio ID {radio_id}"
                        )

                except Exception as e:
                    self.logger.error(f"❌ EXTRACT ERROR Radio ID {radio_id}: {str(e)}")
                    # Continuar con otros IDs en caso de error específico
                    continue

            total_records = len(all_radio_data)
            self.logger.info(
                f"📥 EXTRACT: Extraídos {total_records} registros totales de Radio para {fecha}"
            )

            if total_records == 0:
                self.logger.warning("⚠️ EXTRACT: No se obtuvieron datos de Radio")

            return all_radio_data

        except Exception as e:
            self.logger.error(f"❌ EXTRACT ERROR: {str(e)}")
            raise

    def transform_radio_data(self, raw_data: List[Dict]) -> List[Dict]:
        """
        Transform: Transforma y limpia los datos extraídos de Radio
    ...
    def _apply_data_transformations(self, df: pd.DataFrame) -> pd.DataFrame:
        """Aplica transformaciones específicas a los datos de Radio"""
        ...
        # Transformación de fechas - convertir a objetos date para Radio
        ...
        # Subtitulo: asignar N.A. a valores nulos, vacíos o columna ausente
        ...
        for field in numeric_fields:
            if field in df.columns:
                ...
                # Para campos ID, convertir NaN a None, para otros a 0
                if "Id" in field:
    ...
    def _validate_data_quality(self, df: pd.DataFrame) -> pd.DataFrame:
        """Valida la calidad de los datos transformados de Radio"""
    ...
    def load_radio_data(
        ...
        """
        Load: Carga los datos transformados de Radio en la base de datos
        ...
        try:
            self.logger.info(
                f"💾 LOAD: Iniciando carga de {len(transformed_data)} registros de Radio en base de datos"
            )
            ...
            for record in transformed_data:
                radio_record = db_auditsa_radio(
                    ...
                    Subtitulo=record.get("Subtitulo") or "N.A.",
                ...
                records_to_insert.append(radio_record)
            ...
            self.logger.info(
                f"💾 LOAD: Completada carga de {total_inserted} registros de Radio"
            )
            ...
            ss_error = None
            try:
                ss_inserted = write_to_sqlserver(transformed_data, "radio")
            except Exception as ss_e:
                ss_error = str(ss_e)
                self.logger.error("SQL Server dual-write Radio falló (no-fatal): %s", ss_e)

            return {
                "status": "success",
                "message": f"Datos de Radio cargados exitosamente",
    ...
    def run_etl_pipeline(
        ...
        """
        Ejecuta el pipeline ETL completo para datos de Radio
        ...
        try:
            ...
            self.logger.info(
                f"🚀 ETL PIPELINE: Iniciando proceso de Radio para fecha {fecha}"
            )

            # Extract
            raw_data = self.extract_radio_data(fecha)

            # Transform
            transformed_data = self.transform_radio_data(raw_data)

            # Load
            load_result = self.load_radio_data(transformed_data, db)
            ...
            return {
                ...
                "message": f"ETL de Radio completado exitosamente para fecha {fecha}",
            }
            ...
            return {
                ...
                "error": error_msg,
                "duration_seconds": duration,
                "message": f"Error en ETL de Radio para fecha {fecha}: {error_msg}",
            }
    ...
    def run_etl_pipeline_range(
        ...
        """
        Ejecuta el pipeline ETL para un rango de fechas de Radio
        ...
        try:
            ...
            self.logger.info(
                f"🚀 ETL PIPELINE RANGE: Iniciando proceso de Radio para rango {fecha_inicio} - {fecha_fin}"
            )
            ...
            return {
                ...
                "message": f"ETL de rango de Radio completado: {len(successful_dates)} exitosas, {len(failed_dates)} fallidas",
            }
            ...
            return {
                ...
                "error": error_msg,
                "duration_seconds": duration,
                "message": f"Error en ETL de rango de Radio {fecha_inicio}-{fecha_fin}: {error_msg}",
            }

    def get_etl_status(self, fecha: str) -> Dict[str, Any]:
        """
        Obtiene el estado de los datos de Radio para una fecha específica
        ...
        try:
            ...
            result = (
                db.query(
                    ...
                    func.count(func.distinct(db_auditsa_radio.IdCanal)).label(
                        "unique_channels"
                    ),
                    func.count(func.distinct(db_auditsa_radio.IdMarca)).label(
                        "unique_brands"
                    ),
                    func.count(func.distinct(db_auditsa_radio.IdAnunciante)).label(
                        "unique_advertisers"
                    ),
                )
                .filter(db_auditsa_radio.Fecha == fecha_obj)
                .first()
            )
    ...
    def get_etl_status_range(self, fecha_inicio: str, fecha_fin: str) -> Dict[str, Any]:
        """
        Obtiene el estado de los datos de Radio para un rango de fechas
        ...
        try:
            ...
            result = (
                db.query(
                    ...
                    func.count(func.distinct(db_auditsa_radio.Fecha)).label(
                        "unique_dates"
                    ),
                )
                .filter(
                    db_auditsa_radio.Fecha >= fecha_inicio_obj,
                    db_auditsa_radio.Fecha <= fecha_fin_obj,
                )
                .first()
            )
    ...
    def fix_nulls_in_db(self, db: Optional[Session] = None) -> Dict[str, Any]:
        """
        Limpia valores inválidos en todas las columnas de auditsa_api_radio:
        ...
        try:
            ...
            for col in string_cols:
                result = db_session.execute(text(
                    f'UPDATE public.auditsa_api_radio SET "{col}" = \'N.A.\' '
            ...
            for col in int_cols:
                result = db_session.execute(text(
                    f'UPDATE public.auditsa_api_radio SET "{col}" = 0 WHERE "{col}" IS NULL'
                ))
                total_updated += result.rowcount
            for col in float_cols:
                result = db_session.execute(text(
                    f'UPDATE public.auditsa_api_radio SET "{col}" = 0.0 WHERE "{col}" IS NULL'
                ))
                total_updated += result.rowcount

            db_session.commit()
            self.logger.info(f"🧹 FIX: {total_updated} celdas normalizadas en auditsa_api_radio")
            return {
                "status": "success",
                "records_updated": total_updated,
                "message": f"{total_updated} celdas actualizadas en auditsa_api_radio",
            }
        except Exception as e:
            db_session.rollback()
    ...
    def delete_radio_data(
        self, fecha: str, db: Optional[Session] = None
    ) -> Dict[str, Any]:
        """
        Elimina datos de Radio para una fecha específica en la base de datos mxRepository.source_temp.auditsa_api_radio
        ...
        try:
            ...
            delete_query = text(
                """
                DELETE FROM public.auditsa_api_radio
    ...
    def delete_radio_data_range(
        self, fecha_inicio: str, fecha_fin: str, db: Optional[Session] = None
    ) -> Dict[str, Any]:
        """
        Elimina datos de Radio para un rango de fechas en la base de datos mxRepository.source_temp.auditsa_api_radio
